...
const productSchema = Joi.object({
  ...
  category_id: Joi.string().guid({ version: "uuidv4" }).required(), // UUID validation
...
const createProductController = async (req, res) => {
  try {
    ...
    if (!req.file) return apiResponse(res, 400, false, "Product image is required");
    ...
    return apiResponse(res, 500, false, "Failed to create product", { error: err.message });
  }
};
...
const getProductsController = async (req, res) => {
  try {
    const { page = 1, limit = 20, sortBy = "createdAt", order = "desc", category, search } = req.query;
    const data = await listProducts({ page, limit, sortBy, order, category, search });
    ...
    return apiResponse(res, 500, false, "Failed to fetch products", { error: err.message });
  }
};
...
const getProductController = async (req, res) => {
  try {
    ...
    return apiResponse(res, 500, false, "Failed to fetch product", { error: err.message });
  }
};
...
const updateProductController = async (req, res) => {
  try {
    ...
    return apiResponse(res, 500, false, "Failed to update product", { error: err.message });
  }
};
...
const deleteProductController = async (req, res) => {
  try {
    ...
    return apiResponse(res, 500, false, "Failed to delete product", { error: err.message });
  }
};
...
const createCategoryController = async (req, res) => {
  try {
    ...
    return apiResponse(res, 500, false, "Failed to create category", { error: err.message });
  }
};
...
const getCategoriesController = async (_req, res) => {
  try {
    ...
    return apiResponse(res, 500, false, "Failed to fetch categories", { error: err.message });
  }
};
