...
const protect = async (req, res, next) => {
  ...
  try {
    // ------------------ IP BLOCK CHECK ------------------
    try {
      const blocked = await isIPBlocked(ip); // make sure isIPBlocked uses correct column name
      logger.info(`IP block check for ${ip}: ${blocked}`);
      if (blocked) {
    ...
    let decoded;
    try {
      decoded = verifyToken(token); // verifyToken should match your JWT secret & algorithm
    ...
    try {
      ...
      user = result.rows[0];
      logger.info(`Fetched user: ${user ? user.email : "not found"}`);

      if (!user || !user.is_active) {
        await logLoginAttempt(decoded.id, ip, false);
...
const authorize = (allowedRoles = []) => (req, res, next) => {
  try {
    ...
    if (!allowedRoles.includes(req.user.role)) {
      logger.warn(`User ${req.user.id} denied access. Allowed roles: ${allowedRoles.join(", ")}`);
